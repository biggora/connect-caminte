...
module.exports = function (connect) {
    ...
    function CaminteStore(options, callback) {
        ...
        try {
            ...
            var schema = new Schema(self.options.driver, options.db);
            self.Session = sessionModel(schema, options);
        ...
        return callback && callback(self.Session);
    }
    ...
    CaminteStore.prototype.length = function (callback) {
        this.Session.count({}, function (err, count) {
            if (err) {
                callback && callback(err);
            } else {
    ...
    CaminteStore.prototype.clear = function (callback) {
        this.Session.destroyAll(function () {
            callback && callback();
        });
    };
    ...
    CaminteStore.prototype.all = function (callback) {
        this.Session.all({}, function (err, sessions) {
            if (err) {
                callback && callback(err);
            } else {
